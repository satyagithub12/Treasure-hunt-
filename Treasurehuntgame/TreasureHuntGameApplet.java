...
public class TreasureHuntGameApplet extends JFrame {
    ...
    private void initUI() {
        ...
        distanceLabel = new JLabel("Please select BFS or DFS", SwingConstants.CENTER);
        distanceLabel.setForeground(Color.WHITE);

        bfsButton = new JButton("BFS");
        dfsButton = new JButton("DFS");

        bfsButton.addActionListener(e -> setAlgorithm("BFS"));
        dfsButton.addActionListener(e -> setAlgorithm("DFS"));
    ...
    private void setAlgorithm(String algorithm) {
        distanceLabel.setText("Algorithm set to " + algorithm);
        algorithmSelected = true;
    }

    private void handleMove(KeyEvent e) {
        if (!algorithmSelected) {
            statusLabel.setText("Select BFS or DFS to start moving.");
            return;
        }
        ...
        if (newX >= 0 && newX < GRID_SIZE && newY >= 0 && newY < GRID_SIZE && grid[newX][newY] != OBSTACLE) {
            ...
            statusLabel.setText("Invalid move. Hit a wall or obstacle.");
        }
    }
    ...
    private void startFastBlinkingAnimation() {
        Timer fastBlinkTimer = new Timer(100, e -> {
            playerVisible = !playerVisible;
            updateGrid();
        });
        fastBlinkTimer.setRepeats(true);
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(() -> {
            TreasureHuntGameApplet frame = new TreasureHuntGameApplet();
